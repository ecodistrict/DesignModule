...
const
  ...
  MaxUDPCommandBufferSize              = 512-62;
...
type
  ...
  private
    ...
    procedure SetAddrAndPortStr(const aValue: string);
...
function getaddrinfo(NodeName: PAnsiChar; ServiceName: PAnsiChar; Hints: Paddrinfo; var ppResult: PAddrInfo): Integer; stdcall; external ws2_32DLL name 'getaddrinfo';
procedure freeaddrinfo(ai: PAddrinfo); stdcall; external ws2_32DLL name 'freeaddrinfo';
...
function setsockopt(s: TSocket; level, optname: Integer; optval: PAnsiChar; optlen: Integer): Integer; stdcall; external ws2_32DLL name 'setsockopt';
...
function getsockopt(s: TSocket; level, optname: Integer; optval: PAnsiChar; var optlen: Integer): Integer; stdcall; external ws2_32DLL name 'getsockopt';
...
function getnameinfo(sa: PSockAddr; salen: Integer; host: PAnsiChar; hostlen: Integer; serv: PAnsiChar; servlen: Integer; flags: Integer): Integer; stdcall; external ws2_32DLL name 'getnameinfo';
...
function  GetSocketAddresses           ( const aHost, aService: AnsiString): TSockAddresses;
...
function  SockAddrToURI                ( aSockAddr: TSockAddr): String;
function  URIToSockAddresses           ( const aURI: String): TSockAddresses;
...
begin
  if StartsWith(s, LeftStr) then
    Result := StartStripCnt(s, Length(LeftStr))
  else
    Result := s;
end;
...
begin
  if EndsWith(s, RightStr) then
    Result := EndStripCnt(s, Length(RightStr))
  else
    Result := s;
end;
...
begin
  ...
  begin
    ...
    while (Result > 0) and (Copy(str, Result, Length(SubStr)) <> SubStr) do
      Result := Result - 1;
  end
  else
    Result := 0;
...
procedure TSockAddr.SetAddrAndPortStr(const aValue: string);
var
  a: string;
  p: string;
begin
...
begin
  aistart := nil;
  try
    if getaddrinfo(PAnsiChar(AnsiString(aValue)), PAnsiChar(AnsiString(IntToStr(Port))), nil, aistart)=0 then
    begin
...
function TAddrInfo.FamilyToStr: string;
begin
  Result := AddressFamilyToStr(ai_family);
end;

function TAddrInfo.ProtocolToStr: string;
begin
  Result := AddressProtocolToStr(ai_protocol);
end;

function TAddrInfo.SockTypeToStr: string;
begin
...
begin
  ...
  aistart := nil;
  try
    if getaddrinfo(PAnsiChar(aHost), PAnsiChar(aService), nil, aistart)=0 then
    begin
      ai := aistart;
      repeat
...
begin
  ...
  Result := setsockopt(aSocket, IPPROTO_TCP, TCP_NODELAY, PAnsiChar(@OptVal), SizeOf(OptVal))<>SOCKET_ERROR;
end;
...
begin
  ...
  Result := setsockopt(aSocket, SOL_SOCKET, SO_KEEPALIVE, PAnsiChar(@OptVal), SizeOf(OptVal))<>SOCKET_ERROR;
end;
...
begin
  ...
  if getsockopt(aSocket, SOL_SOCKET, SO_LINGER, PAnsiChar(@Linger), LingerLen)=0 then
  begin
    if Linger.l_onoff=0
    then Result := 0
...
begin
  ...
  Result := setsockopt(aSocket, SOL_SOCKET, SO_LINGER, PAnsiChar(@Linger), SizeOf(Linger))<>SOCKET_ERROR;
end;

function SockAddrToURI(aSockAddr: TSockAddr): String;
begin
  Result := BuildURI(socksprot, aSockAddr.AddrAsStr, IntToStr(aSockAddr.Port), '');
end;
...
begin
  SplitURI(aURI, Protocol, Host, Port);
  if Port<>''
  then Result := GetSocketAddresses(AnsiString(Host), AnsiString(Port))
  else Result := GetSocketAddresses(AnsiString(Host), AnsiString(IntToStr(DefaultPort)));
end;
