...
uses
  {$IFDEF GISDEBUG}
  GisDebug, GisDefs, System.UITypes, //Vcl.Graphics,
  {$ENDIF}
...
type
  ...
  TDLPoint = class
    constructor Create(ax, ay: TDLCoordinate; aValue: TDLValue=NaN; aID: Integer=-1);
  public
    x: TDLCoordinate;
    y: TDLCoordinate;
    Value: TDLValue;
  ...
  public
    ...
    // action
    procedure Clear;
    procedure Triangulate(aMaxEdgeLengthX, aMaxEdgeLengthY: TDLCoordinate; aNodata: TDLValue=NaN; aThreadPool: TMyThreadPool=nil; aParam: Integer=0);
    // inquire
    function PointToTriangle(aPoint: TDLPoint): TDLTriangle;
    ...
    procedure AddPoint(aPoint: TDLPoint; aMaxEdgeLengthX, aMaxEdgeLengthY: TDLCoordinate; var aExtent: TGIS_Extent; aNoData: TDLValue=NaN); overload;
    ...
    procedure AbortTriangulation();
    procedure ReTriangulate(aMaxEdgeLengthX, aMaxEdgeLengthY: TDLCoordinate; aNoData: TDLValue=NaN);
...
begin
  for p := 0 to Count - 1
  do delaunay2.ExtendExtent(Points[p], aExtent);
end;
...
begin
  ...
  for t := 0 to Count - 1 do
  begin
    if (not Triangles[t].Disabled) and (Triangles[t].CheckEdgeLengths(aMaxEdgeLengthX, aMaxEdgeLengthY) or Triangles[t].HasNoData(aNoData)) then
    begin
...
{$IFDEF GISDEBUG}
const
  LineColors: array [0 .. 9] of TColor = (
    TColors.Blue, TColors.Red,
    TColors.Purple,
    TColors.Yellow,
    TColors.Green,
    TColors.Gray,
...
procedure TDLNet.AddPoint(aPoint: TDLPoint; aMaxEdgeLengthX, aMaxEdgeLengthY: TDLCoordinate; var aExtent: TGIS_Extent; aNoData: TDLValue);
...
procedure TDLNet.ReTriangulate(aMaxEdgeLengthX, aMaxEdgeLengthY: TDLCoordinate; aNoData: TDLValue);
begin
  FTriangles.Free;
  FTriangles := TDLTriangleList.Create(True);
  Triangulate(aMaxEdgeLengthX, aMaxEdgeLengthY, aNoData); // todo:
end;
...
procedure TDLNet.Triangulate(aMaxEdgeLengthX, aMaxEdgeLengthY: TDLCoordinate; aNodata: TDLValue; aThreadPool: TMyThreadPool; aParam: Integer);
var
  TopNode: TDLNode;
  {$IFDEF PERFORMANCELOG}
  StartTime: Int64;
